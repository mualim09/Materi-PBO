...
public class SimulatedBank {
    ...
    private Status withdrawal(Message message, Balances balances) {
        ...
        if (accountNumber == 0)
            return new Failure("Rekening Yang anda Masukkan Salah");
    ...
    /**
     * Array of account numbers associated with each card. For each card, there can
     * be three different types of account, which correspond to the names in class
     * AccountInformation. 0 means no account of this type. (Valid card numbers
     * start with 1)
     */
    private static final int ACCOUNT_NUMBER[][] = { { 0, 0, 0 }, // dummies for nonexistent card 0
            { 1, 2, 0 }, { 1, 0, 3 } };

    /**
     * Withdrawals so far today on each card. (Valid card numbers start with 1)
     */
    private static Money WITHDRAWALS_TODAY[] = { new Money(0), // dummy for nonexistent card 0
            new Money(0), new Money(0) };
    private static Money TRANSFER_TODAY[] = { new Money(0), // dummy for nonexistent card 0
            new Money(0), new Money(0) };
    ...
    private Money BALANCE[] = { new Money(0) // dummy for nonexistent account 0
            , new Money(100000000), new Money(50000000)
            // , new Money(5000)
    };
     ...
     * static final.
     */
    private Money AVAILABLE_BALANCE[] = { new Money(0) // dummy for nonexistent account 0
            , new Money(5000000), new Money(10000000)
            // , new Money(5000)
    };
}

...
public abstract class Transaction {
    ...
    /**
     * Create a transaction of an appropriate type by asking the customer what type
     * of transaction is desired and then returning a newly-created member of the
    ...
    public static Transaction makeTransaction(ATM atm, Session session, Card card, int pin)
        ...
        int choice = atm.getCustomerConsole().readMenuChoice("Silahkan pilih jenis transaksi yang ingin dilakukan",
    ...
    /**
     * Peform a transaction. This method depends on the three abstract methods that
     * follow to perform the operations unique to each type of transaction in the
     * appropriate way.
     *
     * @return true if customer indicates a desire to do another transaction; false
     *         if customer does not desire to do another transaction
            ...
            switch (state) {
                case GETTING_SPECIFICS_STATE:
                    ...
                    try {
                        ...
                        doAnotherMessage = "Transaksi terakhir telah dibatalkan";
                ...
                case ASKING_DO_ANOTHER_STATE:
                    ...
                    try {
                        String[] yesNoMenu = { "Ya", "Tidak" };

                        boolean doAgain = atm.getCustomerConsole().readMenuChoice(
                                doAnotherMessage + "Apakah anda ingin melakukan transaksi lain?", yesNoMenu) == 0;
                        return doAgain;
                    } catch (CustomerConsole.Cancelled e) {
                        return false;
                    }
     ...
     * @exception CardRetained              if card was retained due to too many
     *                                      invalid PIN's
     */
    public Status performInvalidPINExtension() throws CustomerConsole.Cancelled, CardRetained {
        ...
        for (int i = 0; i < 3; i++) {
            pin = atm.getCustomerConsole()
                    .readPIN("PIN yang Anda masukan salah\nTolong masukan kembali PIN Anda\n" + "Lalu tekan ENTER");
            atm.getCustomerConsole().display("");
        ...
        atm.getCustomerConsole()
                .display("Kartu Anda telah ditahan\nSilahkan hubungi Bank Uiversitas Gunadarma terdekat");
        try {
            Thread.sleep(5000);
    ...
    public static class CardRetained extends Exception {
         ...
         */
        public CardRetained() {
            super("Kartu ditahan karena melampaui batas kesalahan memasukan PIN");
        }
    }
     ...
     */
    private static final String[] TRANSACTION_TYPES_MENU = { "Tarik Tunai", "Transfer", "Info Saldo" };
